...
export const checkData = (password, confirm, fullname, email, phone) => {
    ...
    if (password === '')
        ...
        return { status: 404, errorString: "Họ tên phải lớn hơn 6 ký tự" };
    else if (fullname.length > 20)
        return { status: 404, errorString: "Họ tên phải tối đa 20 ký tự" };
    else if (email === '')
...
export const checkDataUpdatePassword = (cur, password,confirm) => {
    if (cur === '')
        return { status: 404, errorString: "Mật khẩu hiện tại không được để trống" };
    else if (password === '')
        return { status: 404, errorString: "Mật khẩu mới không được để trống" };
    else if (confirm === '')
        return { status: 404, errorString: "Nhập lại mật khẩu không được để trống" };
    else if (password.length < 8 || password.length > 20)
        return { status: 404, errorString: "Mật khẩu mới phải lớn hơn 8 và nhỏ hơn 20 ký tự" };
    else if (password != confirm)
        return { status: 404, errorString: "Xác nhận mật khẩu không trùng khớp" };
    else
        return { status: 200 };
...
export const checkDataUpdateProfile = (name, phone) => {
    // if(username === '')
    //     return {status: 404, errorString: "Tên đăng nhập không được để trống"};
    if (name === '')
        return { status: 404, errorString: "Họ tên không được để trống" };
